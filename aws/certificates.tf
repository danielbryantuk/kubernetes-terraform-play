data "template_file" "certificates" {
  template = "${file("${var.templates_location}kubernetes-csr.json.tpl")}"
  vars {
...
resource "null_resource" "certificates" {
  ...
  provisioner "local-exec" {
    command = "cd cert-authority; cfssl gencert -initca ../${var.templates_location}/ca-csr.json | cfssljson -bare ca; cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=../${var.templates_location}/ca-config.json -profile=kubernetes kubernetes-csr.json | cfssljson -bare kubernetes"
  }
}
