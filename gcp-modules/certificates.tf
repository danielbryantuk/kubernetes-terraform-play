data "template_file" "certificates" {
  ...
  vars { # todo make this dynamic
    controller0_ip = "${module.kubecontroller.google_compute_instance.controller.0.network_interface.0.address}"
    controller1_ip = "${module.kubecontroller.google_compute_instance.controller.1.network_interface.0.address}"
    controller2_ip = "${module.kubecontroller.google_compute_instance.controller.2.network_interface.0.address}"
    worker0_ip = "${module.kubeworker.google_compute_instance.worker.0.network_interface.0.address}"
    worker1_ip = "${module.kubeworker.google_compute_instance.worker.1.network_interface.0.address}"
    worker2_ip = "${module.kubeworker.google_compute_instance.worker.2.network_interface.0.address}"
